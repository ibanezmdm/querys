...
BEGIN
	...
	SELECT 
		...
		CASE WHEN O.RANGO_OBSOLETOS LIKE '%PRE%'
			THEN CASE WHEN (CONVERT(INT, O.PRECIO_ORIGINAL * O.DESCUENTO / 10) * 10 + 6) < O.PRECIO_VIGENTE
				THEN CONVERT(INT, O.PRECIO_ORIGINAL * O.DESCUENTO / 10) * 10 + 6
				ELSE O.PRECIO_VIGENTE
			END 
			ELSE CASE WHEN (CONVERT(INT, O.PRECIO_ORIGINAL * O.DESCUENTO / 10) * 10 + 7) < O.PRECIO_VIGENTE
				THEN CONVERT(INT, O.PRECIO_ORIGINAL * O.DESCUENTO / 10) * 10 + 7
				ELSE O.PRECIO_VIGENTE
			END
		END NUEVO_PRECIO
	FROM (
		...
		FROM (
			SELECT DISTINCT
				...
				CASE WHEN CONVERT(DATE, GETDATE()) > CONVERT(DATE, O.FECHA_VENCIMIENTO)
					THEN CASE 
