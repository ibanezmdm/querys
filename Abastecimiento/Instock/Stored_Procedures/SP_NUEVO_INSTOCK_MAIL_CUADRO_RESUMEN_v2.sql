	...
	FROM (
		SELECT 
			CASE
				...
				ELSE 'SEM' + CONVERT(VARCHAR,(DATEPART(wk, GETDATE()) + DATEPART(yy, GETDATE())*100) - SEMANA)
			END SEMANA,
			NUM_OH_VALORIZADO,
		...
		FROM (
			...
			*/
			...
			LEFT JOIN [INFORMES3].[dbo].[TIENDAS_ASENTADAS] T
				ON I.COD_LOCAL = T.COD_LOCAL
			WHERE 
				SEMANA BETWEEN DATEPART(wk, GETDATE()) - 4 + DATEPART(yy, GETDATE())*100 AND DATEPART(wk, GETDATE()) - 1 + DATEPART(yy, GETDATE())*100
				AND T.COD_LOCAL IS NULL
			GROUP BY
				SEMANA,
				TOP_500,
