USE [INFORMES3]
GO
/****** Object:  StoredProcedure [dbo].[SP_NUEVO_INSTOCK_MAIL_CUADRO_RESUMEN]    Script Date: 14-11-2019 12:33:16 ******/
SET ANSI_NULLS ON
GO
...
FROM (SELECT 'TOP500' AS CUADRO_RESUMEN
        ...
        ,''+CONVERT(VARCHAR(10),CONVERT(NUMERIC(12,1),SUM([NUM_OH_VALORIZADO])/nullif(SUM([NUM_VTA_SEM_X_PERFIL]),0)*100))+'%'		AS INSTOCK
         ...
         WHERE (TOP_500='TOP500' AND SEMANA=(CASE WHEN DATEPART(ww,GETDATE())<10 THEN (select ''+CAST(DATEPART(YYYY,GETDATE()) AS NVARCHAR(MAX))+'0'+CAST(DATEPART(ww,GETDATE()) AS NVARCHAR(MAX))+'')
